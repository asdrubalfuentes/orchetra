...
- **Fix de campo (2026-09-14b) + IP fija con DNS en los 3 firmwares:** ningún
  equipo con **IP fija** tenía DNS configurado (`WiFi.config()` no traía
  `dns1`/`dns2` — a diferencia de DHCP, que lo recibe del router solo) →
  `github.com` (OTA) y `pool.ntp.org`/`time.google.com` (SNTP, cierre
  automático de día/mes en `nodeIO_master`) fallaban siempre, el 100% de las
  veces, sin ser un problema de la red. Corregido y, de paso, agregado como
  **opción configurable** (antes solo `nodeIO_master` la tenía) en los tres:
  IP fija + Gateway/Máscara/DNS 1/DNS 2 (vacío = DHCP; DNS 1 vacío con IP
  fija usa el propio Gateway, DNS 2 vacío usa `8.8.8.8`).
- **Firmwares migrados y etiquetados:** `nodeIO v1.4.2`, `nodeIO_master
  v1.5.3`, `miHMI v0.5.1` — todos con OTA "GitHub Releases pull". `nodeIO` no
