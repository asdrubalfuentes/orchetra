...
| **v3 — cambio de rumbo** ([`REGISTER_MAP.md` §3.4/§8](REGISTER_MAP.md)) | escalado/totalizador/alarma de proceso al nodo (`nodeIO` ≥ `1.4.0`); gateway gana MAPA A2 (`nodeIO_master` ≥ `1.5.0`) + cierre automático de día/mes por SNTP; LOGO! simplificado a relay + alarmas de enlace/dispositivo (este documento); `miHMI` sin página de rangos + histórico de 6 meses (`miHMI` ≥ `0.5.0`) | **cerrado (2026-09-14) — verificado de punta a punta con hardware real y `tools/mapb_check.py`:** `CONTRACT_VERSION=3` ✓, relay MAPA A2 nivel/caudal/acumulados/`almBits` ✓ (14 bloques Network Input confirmados byte a byte contra `PLC_REGISTER_RECIPE.md §2.4`), fusión de alarmas ✓, `Estado.link_ok` (bit 5, `V17.5`/`V81.5`) ✓ en ambas estaciones. `DI[4]` por FC02 falla el check, pero es **opcional por contrato** (§4.2 de `REGISTER_MAP.md`: "los clientes deben tolerar que FC02 no responda y usar `HR_STATUS`") — ningún cliente real depende de él, no bloquea el cierre. Nivel=Caudal idénticos en banco: no es bug del LOGO! (confirmado que relaya fielmente el MAPA A2 del gateway) — es que el nodo tiene ambos canales con la misma calibración por defecto sin sensor real conectado; se resuelve calibrando en el portal de `nodeIO`. Estación 1 sin verificar con hardware real (sin segundo nodo físico adoptado aún) pero su FBD es simétrico y ya se revisó igual de correcto. |
